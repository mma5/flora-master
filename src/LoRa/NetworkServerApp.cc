...
void NetworkServerApp::initialize(int stage)
{
    if (stage == 0) {
        ...
        evaluateMADRinServer = par("evaluateMADRinServer");
...
void NetworkServerApp::addPktToProcessingTable(LoRaMacFrame* pkt)
{
    ...
    for(uint i=0;i<receivedPackets.size();i++)
    {
        ...
        {
            packetExists = true;
            EV<<"206 The getSrcAddr is "<<cInfo->getSrcAddr()<<" and the getSNIR is "<<math::fraction2dB(pkt->getSNIR())<<" and the getRSSI is "<< pkt->getRSSI()<<" and the source is "<< pkt->getTransmitterAddress()<<endl;
            receivedPackets[i].possibleGateways.emplace_back(cInfo->getSrcAddr(), math::fraction2dB(pkt->getSNIR()), pkt->getRSSI());
          // if (receivedPackets[i].possibleGateways.size()==2){
          //  receivedPacket rcvPktDuplicated;
          //  rcvPktDuplicated.rcvdPacket = pkt;
          //  rcvPktDuplicated.endOfWaiting = new cMessage("endOfWaitingWindow");
          //  rcvPktDuplicated.endOfWaiting->setContextPointer(pkt);
         //   scheduleAt(simTime() + 1.2, rcvPktDuplicated.endOfWaiting);
         //  }
            delete pkt;
            break;
        }
    ...
    if(packetExists == false)
    {
        ...
        EV<<"225 The getSrcAddr is "<<cInfo->getSrcAddr()<<" and the getSNIR is "<<math::fraction2dB(pkt->getSNIR())<<" and the getRSSI is "<< pkt->getRSSI()<<" and the source is "<< pkt->getTransmitterAddress()<<endl;
...
void NetworkServerApp::processScheduledPacket(cMessage* selfMsg)
{
    ...
    if(evaluateMADRinServer)
        {
            evaluateMADR(frame, pickedGateway, SNIRinGW, RSSIinGW);
        }
...
void NetworkServerApp::evaluateMADR(LoRaMacFrame* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW)
{
    bool sendADR = false;
    bool sendADRAckRep = false;
    double SNRm; //needed for ADR
  ...
  //  if(rcvAppPacket->getOptions().getADRACKReq())
   ...
   // if(sendADR || sendADRAckRep)
    //{
    double MaxRSSIinGW = -99999999999;
    for(uint i=0;i<receivedPackets.size();i++)
        {
            if(receivedPackets[i].rcvdPacket->getTransmitterAddress() == pkt->getTransmitterAddress() && receivedPackets[i].rcvdPacket->getSequenceNumber() == pkt->getSequenceNumber())
            {
                for(uint j=0;j<receivedPackets[i].possibleGateways.size();j++)
                {
                       ...
                       // SNIRinGW = std::get<1>(receivedPackets[i].possibleGateways[j]);
                       // pickedGateway = std::get<0>(receivedPackets[i].possibleGateways[j]);
                    if(MaxRSSIinGW < std::get<2>(receivedPackets[i].possibleGateways[j]))
                                    {
                        MaxRSSIinGW = std::get<2>(receivedPackets[i].possibleGateways[j]);
                                    }

                    EV<<"467 The getSrcAddr is "<<std::get<0>(receivedPackets[i].possibleGateways[j])<<" and the getSNIR is "<<std::get<1>(receivedPackets[i].possibleGateways[j])<<" and the getRSSI is "<< std::get<2>(receivedPackets[i].possibleGateways[j])<<" and the source is "<< pkt->getTransmitterAddress()<<endl;

                }
            }
        }
    EV<<"MaxRSSI is "<<MaxRSSIinGW<<endl;
  //  LoRaAppPacket *mgmtPacket = new LoRaAppPacket("ADRcommand");
       // mgmtPacket->setMsgType(TXCONFIG);
      ...
      //  if(sendADR)
        //{
         ...
         //   knownNodes[nodeIndex].calculatedSNRmargin->record(SNRmargin);
          //  int Nstep = round(SNRmargin/3);
          //  LoRaOptions newOptions;
            int orginalSF = pkt->getLoRaSF();
            // Increase the data rate with each step
            int calculatedSF = pkt->getLoRaSF();
              EV<<" Before: calculatedSF is "<<calculatedSF<<endl;
           if ((MaxRSSIinGW >= -137 && MaxRSSIinGW < -134.5) || MaxRSSIinGW <-137 ) calculatedSF=12;//if ((MaxRSSIinGW >= -137 && MaxRSSIinGW < -135) || MaxRSSIinGW <-137 ) calculatedSF=12;
           if (MaxRSSIinGW >= -134.5 && MaxRSSIinGW < -132) calculatedSF=11; // if (MaxRSSIinGW >= -135 && MaxRSSIinGW < -133) calculatedSF=11;//
           if (MaxRSSIinGW >= -132 && MaxRSSIinGW < -129) calculatedSF=10;//if (MaxRSSIinGW >= -133 && MaxRSSIinGW < -130) calculatedSF=10; //
           if (MaxRSSIinGW >= -129 && MaxRSSIinGW < -126) calculatedSF=9;//if (MaxRSSIinGW >= -130 && MaxRSSIinGW < -127) calculatedSF=9;//
           if (MaxRSSIinGW >= -126 && MaxRSSIinGW < -123) calculatedSF=8;//if (MaxRSSIinGW >= -127 && MaxRSSIinGW < -124) calculatedSF=8;//
           if (MaxRSSIinGW > -123) calculatedSF=7; //if (MaxRSSIinGW > -124) calculatedSF=7;//
      ...
      //  if(simTime() >= getSimulation()->getWarmupPeriod())
       ...
       // }
        ...
        frameToSend->setLoRaTP(14);
        frameToSend->setLoRaCF(pkt->getLoRaCF());
        frameToSend->setLoRaSF(pkt->getLoRaSF());//(calculatedSF);//
        frameToSend->setLoRaBW(pkt->getLoRaBW());
        socket.sendTo(frameToSend, pickedGateway, destPort);
            }
    //}
